...
const archieve = {
  ...
  'hidden_kaomoji': [1, 1, 300, '點擊表情 (點擊不會增加你的技術)', true],
  'hidden_speed8x': [1, 1, 300, '速度8x (加速世界)', true],
  'hidden_coins86400': [1, 1, 8640, '金幣到86400', true],
  'hidden_exp5': [1, 1, 3600, '經驗Lv.5', true],
  'hidden_reset': [1, 1, 60, '刪除存檔重新開始', true],
  'hidden_fullphoto': [1, 1, 600, '全圖鑑', true],
  'hidden_fullarch': [1, 1, 600, '全成就', true],
  'hidden_mute': [1, 1, 300, '靜音', true]
};
...
const SAVE_KEY = 'unknown_fishing_game_save_v19';
...
function confirmResetData() {
  if (archieve['hidden_reset'] && archieve['hidden_reset'][0] === 1) {
	...
	addLog(`🏆 隱藏成就解鎖 [${archieve['hidden_reset'][3]}]！獎勵 +${reward} 金幣`);
  }
...
function handleKaomojiClick() {
  ...
  if (archieve['hidden_kaomoji'] && archieve['hidden_kaomoji'][0] === 1) {
	...
	coin += reward;
	playSound('achievement');
	addLog(`🏆 隱藏成就解鎖 [${archieve['hidden_kaomoji'][3]}]！獎勵 +${reward} 金幣`);
  }
}
	...
	if (archieve[key][0] === 1) {
	  ...
	  if (key === 'level_5' && exp >= 3125) {
		...
		if (archieve['hidden_exp5'] && archieve['hidden_exp5'][0] === 1) {
		  ...
		  coin += expReward;
		  playSound('achievement');
		  addLog(`🏆 隱藏成就解鎖 [${archieve['hidden_exp5'][3]}]！獎勵 +${expReward} 金幣`);
		}
	  }

	  if (key === 'hidden_coins86400' && coin >= 86400) conditionMet = true;
	  if (key === 'hidden_fullphoto') {
		const allPhoto = fishDataList.every(fish => (b[fish.id] || 0) > 0);
		if (allPhoto) conditionMet = true;
	  }
	  if (key === 'hidden_fullarch') {
		...
		if (allOtherUnlocked) conditionMet = true;
	  }
	  ...
	  if (conditionMet) {
		...
		reward += rewardAmount;
		playSound('achievement');
		addLog(`🏆 解鎖成就 [${descName.split(' ')[0]}]！獎勵 +${rewardAmount} 金幣`);
	  }
	}
  }
  return reward;
